...
echo "=== 🚀 Setup Docs System Advanced (idempotente) ==="

# 1) Struttura cartelle
mkdir -p scripts docs .github/workflows .github/scripts .git/hooks

# 2) Script STRICT avanzato con più generatori
...
```bash
# Setup completo
bash setup_docs_system_advanced.sh
...
# 5) Makefile avanzato con gestione duplicati intelligente
if [[ -f Makefile ]]; then
    # Rimuovi blocchi esistenti del docs system
...
git add setup_docs_system_advanced.sh scripts/cursor_docs_strict.sh docs/DOCS_SYSTEM_README.md CI_STRICT_STEP_YAML.md Makefile .git/hooks/pre-commit 2>/dev/null || true

if ! git diff --cached --quiet; then
    echo "✅ File pronti per commit"
    echo "💡 Esegui: git commit -m 'feat: add advanced docs system with idempotency'"
else
    echo "ℹ️  Nessuna modifica da committare (sistema già presente)"
fi

echo ""
echo "=== 🎯 Sistema Docs Advanced completato! ==="
